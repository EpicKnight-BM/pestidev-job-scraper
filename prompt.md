...
Added 2026-09-08 to the subagent design (`prompt-v2.md` / `site-processor.md`) after two runs in a
row submitted findings that then bounced at `submit_findings` time — `skippedNonIt` on titles that
were genuinely IT roles by content ("Adatelemzési szakértő", "IT operátor (L2)") but matched none of
the live `job_categories` keywords you have no visibility into, and `skippedDuplicate` on a posting
another source already carried. This file never got the equivalent update — do it here too, since
both designs submit through the same API and hit the same failure.
