...
A run driven by this exact file has already tried imitating `prompt-v2.md`'s orchestrator pattern
by dispatching ~12 parallel `Task` subagents anyway, with none of its budget guardrails — it burned
the account's entire 5-hour usage allowance in one ~22-minute run and was killed by the rate limit
before it ever reached Step 4, losing every finding those subagents had computed (confirmed
2026-09-04; see INCIDENTS.md § Fan-out without a budget). Working through sites and searches
sequentially, yourself, is slower per site but is what actually finishes and submits.
...
copied out of the run instruction into a shell command on every single run); it has been removed.
...
proceed with the run. Name the unregistered `pestidev` connector as the problem in your final report
and stop there; there is no working fallback path.

If an MCP tool call fails at the transport layer, retry it once; if it fails again, STOP and report
the failure plainly rather than the rest of the run. A result with `isError: true` is NOT a
transport failure — that is the API rejecting your payload, carrying the same `too_many_rows` /
`rate_limited` details the REST 413 / 429 bodies do. Handle those with the Step 4 response rules and
...
### Pre-check with `check_titles` before opening a detail page

Added 2026-09-08 to the subagent design after two runs in a row submitted findings that then
bounced at `submit_findings` time for reasons this check now catches first (full story:
INCIDENTS.md § `check_titles` added to catch bounced submissions before a detail-page fetch). This
file never got the equivalent update — do it here too, since both designs submit through the same
API and hit the same failure.
...
lookup — so a title that would bounce later gets caught here first, from the title alone, with no
detail-page fetch spent on it.

For every site you touch, after the `activeTitlesByCompany` skip above and before opening a single
detail page, batch every REMAINING listing title through it in ONE call per company:

```
check_titles({ candidates: [{ title: "<listing title>", company: "<company>" }, ...] })
```

For each result: `verdict: "reject"` → do not open its detail page, do not evaluate it against the 6
filters, do not include it in `findings`. It still counts toward the honest `postingsFound` total —
say how many you dropped this way, and why (`reasons`), in your final report (e.g. "3 of 9 dropped by
check_titles: 2 non_it_title, 1 duplicate"). `verdict: "keep"` → evaluate it against the 6 filters
normally.

If the tool is unreachable or errors, do not stall on it — evaluate every remaining posting yourself
against the 6 filters as if this step had not existed, and say so plainly in your final report. This
is a cost-saving pre-check, not a gate you are structurally required to pass through.
...
One unresponsive page can consume an entire run — a hung fetch with no way to interrupt it can cost
a whole run's submission (confirmed 2026-08-17; see INCIDENTS.md § One unresponsive page consuming
a whole run).
...
`nixstech` is in the Set). Also remove anything in the STRICT exclusion list below, matching on the
bare domain/company there since that list has no slugs of its own.
...
Some ATS platforms mint a fresh random suffix for a posting's URL on every crawl or every publish —
the posting did not change, only its URL did. Treated naively ("any URL not in `storedListingUrls`
is new"), this creates a fresh duplicate row on the live board every single time the URL rotates,
because `(source, url)` is the database row identity — the API has no way to know two different URL
strings are the same posting (confirmed 2026-09-02 on Knorr-Bremse's joinus.hu portal; see
INCIDENTS.md § URL rotation vs. a genuinely new posting).

Before evaluating a URL that looks new: run `timeout 5 sh scripts/strip-url-tail.sh "<url>"` on it
and on every stored URL. An exact match between the outputs means this is the SAME posting
rotating, not a new one — do not re-evaluate or resubmit it, just carry the current URL forward in
`listingUrls` so next run's diff has the fresh value. Only what survives this check is a genuine new
posting worth opening.
...
**General lesson from the same audit — don't conclude "unreachable" from a first glance at the
homepage/board.** Before writing any company off as JS-rendered: (1) view-source the page as PLAIN
TEXT and actually scan for `<a href>` links to job detail URLs before assuming there are none —
...
you filter) were the SAME root failure: you evaluated SOME of a site's postings, submitted the ones
that qualified, and reported it as done — without ever knowing how many postings the site actually
had. "I found some and evaluated them" is not a completion signal. It looks identical whether you
found 2 of 2 or 2 of 8.
...
A first-pass miss isn't permanent — keep retrying newly-discovered/unchecked companies rather than
concluding "no career page" after a single failed guess (confirmed 2026-08-01 — two companies' real
career pages were missed entirely on the first pass; see INCIDENTS.md § Career page never reached
at all). Before giving up on a company, work through all of these:
...
**If the career page you find is itself a SEARCH/FILTER interface** (query-string driven — a `?location=`, `?category=`, `locationsearch=`, or similar param already applied by default), the loaded default view is often pre-scoped and NOT the full list. Actively broaden it: strip location/category params to see the unfiltered result set, and check the page's own filter UI for OTHER category values/ids you haven't tried (e.g. a numbered `category=<n>` param implies sibling categories exist — try neighboring numbers or read the filter dropdown's own option list) — evaluate the union of everything reachable, not just whatever loaded first (confirmed miss, 2026-08-01 — see INCIDENTS.md § karrier.4iggroup.hu — a pre-scoped search/filter listing).
...
**Do not stop after the first couple of qualifying links you notice.** When a listing page's fetched HTML already contains a full set of per-job links (no JavaScript needed — you can see them directly in the raw fetch), read and judge EVERY one of them against the 6 filters before deciding what to submit. A link that was already sitting in the HTML you fetched and got skipped anyway is the single most common, and most avoidable, way real findings get missed (see INCIDENTS.md § vector.hu — links sitting unread in an already-fetched page).

**If a listing page's plain fetch shows NO per-job links at all** (the page is client-side/JS-rendered — a Webflow/React/Vue job board with no server-rendered cards, common when the raw HTML has no job titles anywhere in it), before giving up on that company try the site's `sitemap.xml`: check `robots.txt` for a `Sitemap:` line, or just fetch `<domain>/sitemap.xml` directly, and filter its URLs for career/job/karrier/állás paths. Sites that need JavaScript to render their listing page often still list every individual posting's own URL in the sitemap as a plain server-rendered page (see INCIDENTS.md § Never conclude "unreachable" from a first glance for a confirmed case).

**If a listing links directly to PDF files instead of HTML pages** (some companies post openings as downloadable PDFs, e.g. "Álláshirdetés_<role>.pdf" — see INCIDENTS.md § keler.hu — PDF-only listing). This is still a normal, workable posting: the PDF's own URL is the row identity same as any HTML page, read the PDF content for the title/requirements/level exactly like you would a webpage, and evaluate it against the same 6 filters below. Don't treat "the listing only has PDF links" as a dead end — enumerate every PDF the same way you'd enumerate HTML detail pages.
...
1. **Real, distinct, navigable URL per posting.** Reject any site where several different job titles share one page/anchor with no distinct detail URL per job. The URL is the database row identity — a shared URL would silently overwrite a different job. Note: a single posting that legitimately describes ONE role with two variants under one shared application form (e.g. a ".NET/JAVA fejlesztő gyakornok" internship page with sub-sections for each track but one shared apply form) is still just ONE posting — submit it once under its one URL, don't treat the sub-section headings as separate jobs. But if a single page lists genuinely DIFFERENT roles — different titles, different requirements, different apply targets — and each one has its own in-page anchor id (e.g. `example.com/careers#ai-integration-specialist`) or its own distinct apply link/email, that anchor IS a distinct identity: use `pageURL#anchor` as that job's row-identity URL and evaluate/submit each role separately. Only reject the whole page when postings are truly indistinguishable (same anchor, same apply target, no way to tell which role you'd even be applying to) — confirmed miss, 2026-08-01, see INCIDENTS.md § Anchored roles on one page treated as one posting. **The opposite mistake also happens: the SAME real opening listed under several separate, fully-distinct URLs on one company's own listing page** — not query-string variants, not in-page anchors, genuinely separate pages (e.g. numbered slug siblings like `/java-developer/`, `/java-developer-2/`, `/java-developer-3/`). This is not several openings, it's one opening re-listed (some sites do this to keep an ad looking "fresh," or the listing page itself just links the same card twice). When two or more entries on the SAME listing page share the same title and company and, as far as you can tell, the same description/requirements, treat them as ONE posting: submit only the first/lowest-numbered one and skip the rest, even though each has its own genuinely working URL. This is specifically a SAME-RUN problem — the `activeTitlesByCompany` lookup (see above, "Skip postings the database already has") only protects against re-finding a title across SEPARATE runs, it does nothing for duplicate entries discovered together in ONE enumeration, so you have to catch it yourself while reading the listing, before submitting (confirmed case, 2026-07-29 — see INCIDENTS.md § Numbered URL siblings re-listing one opening).
...
3. **IT/software-relevant title** — developer, engineer, QA/tester, **test automation / automated testing engineer** (a distinct, actively-worth-finding role — don't treat it as a lesser variant of "developer" and skip past it), DevOps, data, sysadmin/infra, IT business analyst, **rendszerszervező** (Hungarian for systems analyst/organizer — a real, common IT-adjacent title, treat it the same as "analyst"/"business analyst", not as generic admin). A plain "Business Analyst" (no literal "IT" prefix in the title) still qualifies when the company itself is fundamentally a software/tech business (an insurtech, fintech, SaaS company, etc.) — judge from what the company actually builds, don't require the exact phrase "IT business analyst" (missed example, 2026-08-01: an insurtech's plain "Business analyst" role should have qualified on this basis). Reject sales, marketing, HR, generic admin, and pure business/design-only roles at companies that are NOT themselves tech/software businesses. **Watch for false positives from a bare keyword match** — a title like "Biztonsági Munkatárs" (security staff) at a transport/logistics company can be a PHYSICAL security/guard role (vagyonőr, gazdaságvédelem), not IT/cybersecurity — and "Hálózatszervezési és üzemeltetési munkatárs" at a postal company can mean organizing the physical POST-OFFICE BRANCH network, not IT networking (2026-08-04 audit; see INCIDENTS.md § Never conclude "unreachable" from a first glance for the audit that also caught this). Read enough of the body to confirm the role is actually about computers/software/IT infrastructure before submitting, don't trust a keyword hit on "biztonsági"/"hálózat" alone. **The API re-checks this filter on the TITLE ALONE, so a title with no recognisable IT word gets dropped no matter how IT-relevant the body is.** `check_titles` above already checks this authoritatively — a title that came back `verdict: "keep"` needs no further guessing here. **Fallback only** (`check_titles` was unreachable/errored, or you evaluated a posting without batching it through check_titles first): guess from the same word list — developer / engineer / fejlesztő / tester / tesztelő / QA / DevOps / rendszergazda / adatbázis / analyst / rendszerszervező. No match despite an IT-relevant body → still submit it, the API's own backstop is what actually decides, but flag it plainly in your final report so a human can add the shape here if it bounces. Two confirmed misses worth knowing this fallback's limits from (full story: INCIDENTS.md § Known API-rejected title shapes): "Közmű SAP szakértő" (2026-08-24, MVM Informatika Zrt.) — defensible on the body, SAP IS-U application support, but "közmű szakértő" carries no IT token — and "Szoftverüzemeltető" (2026-09-08, Direktor Szoftver Kft.) — unambiguously IT ops on the body, but neither "szoftver" nor "üzemeltető" alone is a recognised token; the same shape applies to any `<noun> + üzemeltető` title unless the noun itself is one of the recognised tokens.
...
Call `submit_findings` with the payload as its arguments — the same three keys, the same shapes, exactly as the field rules below describe:
...
Send it ONCE — a tool call that returned a result has been applied, and re-sending it double-counts against the upload budget.
...
- `technologies` — comma-joined, but ONLY from this EXACT fixed list (the same recognized-keyword set every hand scraper on this board uses, `TECH_KEYWORDS` in `netlify/functions/_tech_keywords.js`, re-exported by `_experience_core.mjs` — a row with a technology label outside this list is inconsistent with every other source on the board and gets manually corrected after the fact; see INCIDENTS.md § Tech-label drift):
  ...
  **★ NEW — spoken/written language is part of this same field, and every hand-written scraper on the board tags it while you currently don't.** The last block of the list above (`Hungarian`, `English`, `German`, …) covers language, not just tech, and the site's own scrapers add it two ways: (1) the posting explicitly states a language requirement ("angol nyelvtudás", "English: B2", "tárgyalási szintű német") — add that language's label; (2) the posting's own body text is itself predominantly WRITTEN in a given language, with no explicit requirement line at all — add that language's label too. You read the whole posting yourself, so judge (2) directly instead of guessing from a handful of keywords the way the site's own heuristic has to: if the ad body reads as ordinary Hungarian prose, add `Hungarian`; if it reads as ordinary English prose, add `English`; and so on. Most postings on this board are Hungarian by default, so an ordinary Hungarian posting with zero language-requirement line should still get `Hungarian`, exactly the way an English-written posting with no requirement line still gets `English`. Comma-join the language label(s) into the same `technologies` field alongside any real tech labels — there is no separate field for it.
...
- `rejected` — ONLY for sites that can never work regardless of timing (JS-rendered ATS, no per-job URL, wrong vertical, aggregator, already-covered domain, or a board the site's own `ats-crawl` source already harvests). Send it as an array of `{"slug":..., "domain":..., "company":..., "reason":...}` objects — **never a free-text string; the API silently drops any `rejected` entry that isn't an object.** `slug` is required and must be the exact same slug you use in `sitesChecked`/`findings` for this company, since it's the only field `permanentlyRejected` is matched on. Never put a site here just because it has no fit today — that belongs in `sitesChecked`. Entries here are permanent and never re-checked. **Send a site here the FIRST time you reject it permanently and never again** — if `permanentlyRejected` already names that `slug`, re-sending changes nothing and just accumulates near-duplicate entries for one company. And **never send the same company under both `sitesChecked` and `rejected`**: `sitesChecked` refreshes exactly what `rejected` is meant to retire, which is how a permanently-rejected site stays in rotation forever.
...
- A JSON-RPC error, or a tool call that does not come back at all, is a transport-level failure —
  retry once, then STOP and report it plainly. Never silently give up: a run whose submission failed
  produced NOTHING, and saying otherwise is a false report. Print the payload per **Final output**
  below so the run stays replayable.
...
lost this way; see INCIDENTS.md § Lost work after a failed submission with no printed payload).
